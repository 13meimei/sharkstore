...
//		});
  ...
  $scope.checkShardHide = function(spaceroot){
	...
	$scope.changeLeaderInstance = function(rangeLeaderRoot){
		var clusterId = $('#clusterId').val();
		swal({
	    	  title: "切换为主?",
	    	  type: "warning",
	    	...
	    	//执行ajax提交
	    		$.ajax({
	    	        ...
	    	        data:{
	    	        	"clusterId":clusterId,
	    	        	"peerId":rangeLeaderRoot.node.id,
	    	        	"rangeId":rangeLeaderRoot.id
	    	        },
	    			success: function(data){
	    				if(data.attach.code === 0){
	...
	$scope.transferInstance = function(rangeLeaderRoot){
		var clusterId = $('#clusterId').val();
		var dbName = $("#dbName").val();
		var tableName = $("#tableName").val();
		swal({
