...
void DataServer::nodeHeartbeat() {
    ...
    FLOG_DEBUG("Send NodeHeartbeat: %s", req.ShortDebugString().c_str());

    context_->master_client->AsyncNodeHeartbeat(req);
}
