...
func (t *Tree) isBST() bool {
	...
	min, _ := t.Min()
	max, _ := t.Max()
	return t.root.isBST(min, max)
}

// Are all the values in the BST rooted at x between min and max,
// and does the same property hold for both subtrees?
func (n *node) isBST(min, max compare.Comparable) bool {
	if n == nil {
		return true
	}
	if n.key.Compare(min) < 0 || n.key.Compare(max) > 0 {
		return false
	}
...
func (n *node) is23_234() bool {
	...
	if n.getColor() == red && n.left.getColor() == red {
		return false
	}
...
func (t *Tree) isBalanced() bool {
	...
	var cblack int // number of black links on path from root to min
	for x := t.root; x != nil; x = x.left {
		if x.getColor() == black {
			cblack++
		}
	}
	return t.root.isBalanced(cblack)
}

// Does every path from the root to a leaf have the given number
// of black links?
func (n *node) isBalanced(cblack int) bool {
	if n == nil && cblack == 0 {
		return true
	} else if n == nil && cblack != 0 {
		return false
	}
	if n.getColor() == black {
		cblack--
	}
	return n.left.isBalanced(cblack) && n.right.isBalanced(cblack)
}

type compIntUpper int

func (ci compIntUpper) Compare(i compare.Comparable) (c int) {
	switch i := i.(type) {
	case compIntUpper:
		c = int(ci) - int(i)
	case compare.CompInt:
		c = int(ci) - int(i)
	}
	if c == 0 {
		return 1
...
func describeTree(n *node, char, color bool) string {
	...
	follow = func(n *node) {
		...
		if n.key != nil {
			if char {
				s = append(s, rune(n.key.(compare.CompRune)))
			} else {
				s = append(s, []rune(fmt.Sprintf("%d", n.key))...)
			}
			if color {
				s = append(s, []rune(fmt.Sprintf(" %v", n.getColor()))...)
			}
		}
	}
	if n == nil {
...
func (s *S) SetUpSuite(c *check.C) {
	mode := []string{LLRB234: "Top-Down 2-3-4", LLRB23: "Bottom-Up 2-3"}
...
func (s *S) TestNilOperations(c *check.C) {
	...
	min, _ := t.Min()
	max, _ := t.Max()
	c.Check(min, check.Equals, nil)
	c.Check(max, check.Equals, nil)
	if testMode == LLRB234 {
		return
	}
	t.DeleteMin()
	c.Check(*t, check.Equals, Tree{mode: testMode})
	t.DeleteMax()
	c.Check(*t, check.Equals, Tree{mode: testMode})
}

func (s *S) TestInsertion(c *check.C) {
	min, max := compare.CompRune(0), compare.CompRune(1000)
	t := New(testMode)
	for i := min; i <= max; i++ {
		t.Insert(i, nil)
	...
	mi, _ := t.Min()
	mx, _ := t.Max()
	c.Check(mi, check.Equals, compare.CompRune(min))
	c.Check(mx, check.Equals, compare.CompRune(max))
}

func (s *S) TestDeletion(c *check.C) {
	min, max := compare.CompRune(0), compare.CompRune(10000)
	e := int(max-min) + 1
	t := New(testMode)
	for i := min; i <= max; i++ {
		t.Insert(i, nil)
...
func (s *S) TestGet(c *check.C) {
	min, max := compare.CompRune(0), compare.CompRune(100000)
	t := New(testMode)
	for i := min; i <= max; i++ {
		if i&1 == 0 {
			t.Insert(i, nil)
	...
	for i := min; i <= max; i++ {
		rk, _ := t.Get(i)
		if i&1 == 0 {
			c.Check(rk, check.Equals, compare.CompRune(i)) // Check inserted elements are present.
		} else {
			c.Check(rk, check.Equals, compare.Comparable(nil)) // Check inserted elements are absent.
		}
	}
}
...
func (s *S) TestFloor(c *check.C) {
	...
	for i := min; i <= max; i++ {
		rk, _ := t.Floor(i)
		if i&1 == 0 {
			c.Check(rk, check.Equals, compare.CompRune(i)) // Check even Floors are themselves.
		} else {
			c.Check(rk, check.Equals, compare.CompRune(i-1)) // Check odd Floors are the previous number.
		}
	}
	rk, _ := t.Floor(min - 1)
	c.Check(rk, check.Equals, compare.Comparable(nil))
}
...
func (s *S) TestCeil(c *check.C) {
	...
	for i := min; i < max; i++ {
		rk, _ := t.Ceil(i)
		if i&1 == 1 {
			c.Check(rk, check.Equals, compare.CompRune(i)) // Check odd Ceils are themselves.
		} else {
			c.Check(rk, check.Equals, compare.CompRune(i+1)) // Check even Ceils are the next number.
		}
	}
	rk, _ := t.Ceil(max + 1)
	c.Check(rk, check.Equals, compare.Comparable(nil))
...
func (s *S) TestUpper(c *check.C) {
	min, max := compare.CompInt(0), compare.CompInt(100000)
	t := New(testMode)
	for i := min; i <= max; i++ {
		if i&1 == 1 { // Insert odd numbers only.
			t.Insert(i, nil)
	...
	for i := min; i < max-1; i++ {
		rk, _ := t.Ceil(compIntUpper(i))
		if i&1 == 1 {
			c.Check(rk, check.Equals, compare.CompInt(i+2)) // Check odd Uppers are the next odd.
		} else {
			c.Check(rk, check.Equals, compare.CompInt(i+1)) // Check even Uppers are the next number.
		}
	}
	rk, _ := t.Ceil(compIntUpper(max + 1))
	c.Check(rk, check.Equals, compare.Comparable(nil))
}
...
func (s *S) TestRandomlyInsertedGet(c *check.C) {
	...
	for i := 0; i < count; i++ {
		v := compare.CompRune(rand.Intn(max))
		t.Insert(v, nil)
		verify[rune(v)] = struct{}{}
	}
	// Random fetch order - check only those inserted.
	for v := range verify {
		rk, _ := t.Get(compare.CompRune(v))
		c.Check(rk, check.Equals, compare.CompRune(v)) // Check inserted elements are present.
	}
	// Check all possible insertions.
	for i := compare.CompRune(0); i <= compare.CompRune(max); i++ {
		rk, _ := t.Get(i)
...
func (s *S) TestRandomInsertion(c *check.C) {
	...
	for i := 0; i < count; i++ {
		r := rand.Intn(max)
		t.Insert(compare.CompRune(r), nil)
		failed := false
		failed = failed || !c.Check(t.isBST(), check.Equals, true)
		failed = failed || !c.Check(t.is23_234(), check.Equals, true)
		failed = failed || !c.Check(t.isBalanced(), check.Equals, true)
...
func (s *S) TestRandomDeletion(c *check.C) {
	var (
		count, max = 100000, 1000
		r          = make([]compare.CompRune, count)
		t          = New(testMode)
	)
	for i := range r {
		r[i] = compare.CompRune(rand.Intn(max))
		t.Insert(r[i], nil)
	}
	for _, v := range r {
		t.Delete(v)
...
func (cs compStructUpper) Compare(s compare.Comparable) (c int) {
...
func (s *S) TestDeleteMinMax(c *check.C) {
	var (
		min, max = compare.CompRune(0), compare.CompRune(10)
		t        = New(testMode)
		dI       int
	)
	for i := min; i <= max; i++ {
	...
	for i, m := 0, int(max); i < m/2; i++ {
		...
		min++
		rk, _ := t.Min()
		failed = !c.Check(rk, check.Equals, min)
		failed = failed || !c.Check(t.isBST(), check.Equals, true)
		failed = failed || !c.Check(t.is23_234(), check.Equals, true)
		failed = failed || !c.Check(t.isBalanced(), check.Equals, true)
		if failed {
		...
		max--
		rkx, _ := t.Max()
		failed = !c.Check(rkx, check.Equals, max)
		failed = failed || !c.Check(t.isBST(), check.Equals, true)
		failed = failed || !c.Check(t.is23_234(), check.Equals, true)
		failed = failed || !c.Check(t.isBalanced(), check.Equals, true)
		if failed {
...
func (s *S) TestRandomInsertionDeletion(c *check.C) {
	...
	for i := 0; i < count; i++ {
		...
		if rand.Float64() < 0.5 {
			rI = rand.Intn(max)
			t.Insert(compare.CompRune(rI), nil)
			verify[rI] = struct{}{}
...
func (s *S) TestDeleteRight(c *check.C) {
	type target struct {
		min, max, target compare.CompRune
	}
	for _, r := range []target{
		...
		if *genDot && t.Len() <= *dotLimit {
			dotString = dot(t, strings.Replace(
				fmt.Sprintf("TestDeleteRight_%s_before_del_%d_%d_%d", modeName[testMode], r.min, r.max, r.target),
				"-", "_", -1))
		}
		t.Delete(r.target)
		...
		if !ok && *genDot && t.Len() < *dotLimit {
			...
			err = dotFile(t, strings.Replace(
				fmt.Sprintf("TestDeleteRight_%s_after_del_%d_%d_%d", modeName[testMode], r.min, r.max, r.target),
				"-", "_", -1), "")
			if err != nil {
				c.Errorf("Dot file write failed: %v", err)
			}
...
var (
	modeName = []string{LLRB234: "TD234", LLRB23: "BU23"}
...
func dot(t *Tree, label string) string {
	...
	follow = func(n *node) {
		...
		c := fmt.Sprintf("%d[label = \"<Left> |<Elem> %v|<Right>\"];", id, n.key)
		if n.left != nil {
			c += fmt.Sprintf("\n\t\tedge [color=%v,arrowhead=%s]; \"%d\":Left -> \"%d\":Elem;",
				n.left.getColor(), arrows[n.left.getColor()], id, uintptr(unsafe.Pointer(n.left)))
			follow(n.left)
		}
		if n.right != nil {
			c += fmt.Sprintf("\n\t\tedge [color=%v,arrowhead=%s]; \"%d\":Right -> \"%d\":Elem;",
				n.right.getColor(), arrows[n.right.getColor()], id, uintptr(unsafe.Pointer(n.right)))
			follow(n.right)
		}
		s = append(s, c)
	}
...
func (s *S) TestDo(c *check.C) {
	...
	var result compInts
	f := func(c compare.Comparable, value interface{}) (done bool) {
		result = append(result, c.(compare.CompInt))
		return
	}
	killed := t.Do(f)
	sort.Sort(values)
...
func (s *S) TestDoRange(c *check.C) {
	values := append(compInts(nil), values...)
	lo, hi := compare.CompInt(0), compare.CompInt(100)
...
func (s *S) TestDoMatch(c *check.C) {
	...
	sort.Sort(values)
	target := values[elem]
	f := func(r compare.Comparable, value interface{}) (done bool) {
...
func BenchmarkInsert(b *testing.B) {
	t := New(testMode)
	for i := 0; i < b.N; i++ {
		t.Insert(compare.CompInt(b.N-i), nil)
	}
}
...
func BenchmarkInsertNoRep(b *testing.B) {
	t := New(testMode)
	for i := 0; i < b.N; i++ {
		t.Insert(compIntUpper(b.N-i), nil)
	}
}
...
func BenchmarkMin(b *testing.B) {
	...
	t := New(testMode)
	for i := 0; i < 1e5; i++ {
		t.Insert(compare.CompInt(i), nil)
	}
	b.StartTimer()
	var m compare.Comparable
	for i := 0; i < b.N; i++ {
		m, _ = t.Min()
	}
	_ = m
