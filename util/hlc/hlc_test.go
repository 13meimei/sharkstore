...
func ExampleNewClock() {
	...
	t := timestamp.Timestamp{WallTime: UnixNano()}
	...
	if t.WallTime-s.WallTime > 0 {
		log.Fatal("HLC timestamp %d deviates from physical clock %d", s, t)
	}
...
func TestHLCClock(t *testing.T) {
	...
	expectedHistory := []struct {
		...
		{5, SEND, nil, timestamp.Timestamp{WallTime: 5, Logical: 0}},
		{6, SEND, nil, timestamp.Timestamp{WallTime: 6, Logical: 0}},
		{10, RECV, &timestamp.Timestamp{WallTime: 10, Logical: 5}, timestamp.Timestamp{WallTime: 10, Logical: 6}},
		// Our clock mysteriously jumps back.
		{7, SEND, nil, timestamp.Timestamp{WallTime: 10, Logical: 7}},
		// Wall clocks coincide, but the local logical clock wins.
		{8, RECV, &timestamp.Timestamp{WallTime: 10, Logical: 4}, timestamp.Timestamp{WallTime: 10, Logical: 9}},
		// Wall clocks coincide, but the remote logical clock wins.
		{10, RECV, &timestamp.Timestamp{WallTime: 10, Logical: 99}, timestamp.Timestamp{WallTime: 10, Logical: 100}},
		// The physical clock has caught up and takes over.
		{11, RECV, &timestamp.Timestamp{WallTime: 10, Logical: 31}, timestamp.Timestamp{WallTime: 11, Logical: 1}},
		{11, SEND, nil, timestamp.Timestamp{WallTime: 11, Logical: 2}},
	}

	var current timestamp.Timestamp
