...
func (c *Clock) getPhysicalClockLocked() int64 {
	...
	if c.mu.lastPhysicalTime != 0 {
		...
		if interval > int64(c.maxOffset/10) {
			...
			log.Warn("backward time jump detected (%f seconds)", float64(newTime-c.mu.lastPhysicalTime)/1e9)
		}
	}
...
func (c *Clock) Now() timestamp.Timestamp {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Clock) Update(rt timestamp.Timestamp) timestamp.Timestamp {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	if rt.WallTime > c.mu.timestamp.WallTime {
		...
		if c.maxOffset > 0 && offset > c.maxOffset {
			log.Warn("remote wall time is too far ahead (%s) to be trustworthy - updating anyway", offset)
		}
