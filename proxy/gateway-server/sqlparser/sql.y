...
%token <empty> ALL DISTINCT AS EXISTS NULL ASC DESC VALUES INTO DUPLICATE KEY DEFAULT SET LOCK GLOBAL SESSION LEVEL
...
// Transaction Tokens
%token <empty> BEGIN START TRANSACTION COMMIT ROLLBACK ISOLATION
%token <empty> READ WRITE ONLY REPEATABLE COMMITTED UNCOMMITTED SERIALIZABLE
...
%type <bytes2> comment_opt comment_list admin_arg_list
%type <str> union_op
...
%type <str> table_opt
%type <empty> transaction_char transaction_chars isolation_level	
...
%type <statement> describe_statement 
...
|	SET GLOBAL TRANSACTION transaction_chars
	{
		$$ = &Set{
			Exprs: UpdateExprs{
	            &UpdateExpr{
...
|	SET SESSION TRANSACTION transaction_chars
	{
		$$ = &Set{
			Exprs: UpdateExprs{
	            &UpdateExpr{
...
admin_statement:
  ADMIN ID '(' admin_arg_list ')'
  {
    $$ = &Admin{Command: $2, Args: $4}
  }

describe_statement:
  DESCRIBE ID
  {
    $$ = &Describe{TableName: $2}
  }
