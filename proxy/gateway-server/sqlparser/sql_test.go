...
func TestDescribe(t *testing.T) {
	...
	if !ok {
		t.Fatalf("expected describe statement. actual: %T", stmt)
	}

	t.Log("describe table ", string(desc.TableName))

	if string(desc.TableName) != "abc" {
		t.Fatalf("expected tableName=abc, actual: %v", desc.TableName)
	}
}
