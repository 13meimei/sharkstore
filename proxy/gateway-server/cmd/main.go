...
stat = &Stat{}
	...
	ip := getIp()
	for concur:=0;concur<s.GetCfg().Threads;concur++ {
		if s.GetCfg().BenchMark & 1 == 1 {
			go insertTestData(s ,concur,10000,ip)
		}
		if s.GetCfg().BenchMark & 2 == 2 {
...
func selectTest(s *server.Server,threadNo, total int,ip string){
	...
	for {
		no := r.Intn(s.GetCfg().SendNum)
		user_name := getUserName(no,ip,threadNo)
		h := hash(user_name)%16384
		pks["user_name"] = user_name
		pks["h"] = h
		reply := api.Select(s,s.GetCfg().TestDB,s.GetCfg().TestTable,fields,pks)
		log.Debug("%v",reply)
...
func insertTestData(s *server.Server,threadNo, total int,ip string){
	...
	real_name := randomString(s.GetCfg().TestDataLen)
	pass_word := "pw"
	var i int

	for i =0;i < s.GetCfg().SendNum ;i++ {
		...
		for b:=1;b< s.GetCfg().TestBatch ;b++ {
			row := make([]interface{},0)
			row = append(row,h)
		...
		reply := api.Insert(s,s.GetCfg().TestDB,s.GetCfg().TestTable,fields,rows)
		log.Debug("%v",reply)
		if reply.Code == 0 {
