...
func TestMasterCommand(testing *testing.T) {
	b := &Boomer{MasterAddr: "192.168.183.67:8887",
	ClusterId: 1,
	ClusterToken: "test",
	Database: "boom_test",
	Table: "test"}
	...
	if !ok {
		fmt.Println("select down peer failed")
		return
	}
	...
	args.Add("d", fmt.Sprintf("%v", d))
	args.Add("s", fmt.Sprintf("%v", s))
	res := b.delPeerCommand(cli, args)
	fmt.Println("del peer command: ", res)
}
